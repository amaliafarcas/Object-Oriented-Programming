...
UserServices::UserServices(Repository& repo):
	...
	watchlist{ std::vector<Movie>(0) }, index{ 0 },
	writer{ nullptr },
	writeMode{ "" },
	movies{ MovieList() }
{
...
void UserServices::FilterByGenre(std::string genre)
{
	...
	for (int i = 0; i < movielist.size(); i++)
		...
		{
            //std::cout<<movielist[i].GetGenre()<<" ";
			movielist.erase(movielist.begin()+i);
			i--;
		}
    //std::cout<<movielist.size();
	index = 0;
}
...
void UserServices::WatchMovie(std::string title, bool rate)
{
    ...
    auto it = std::find_if(this->watchlist.begin(), this->watchlist.end(),
                           [title](Movie m) { return m.GetTitle() == title; });
    if (it<=watchlist.end())
        watchlist.erase(it);
	if (!found)
		throw std::exception();//"No such movie in your watch list");
...
std::vector<Movie> UserServices::GetMovieList() const
{
    return movielist;
}
