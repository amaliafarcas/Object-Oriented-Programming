...
template<typename T>
inline void DynamicVector<T>::RemoveElement(size_t position)
{
	if (this->elements == nullptr) return;

	if (position > this->currentSize) return;

	for (size_t i = position; i < this->currentSize-1; ++i)
...
public:
	...
	static void TestRemovingAndSubscript();
	static void TestAssignmentAndEquality();
};